...
def export_window(file_path):
    ...
    for course_name in current_times.keys():
        course_names_column.append([sg.Text(course_name)])
        weekly_times_column.append([sg.Text(str(current_times[course_name]))])
    ...
    layout = [
        [sg.Column(course_names_column), sg.Column(weekly_times_column), sg.VerticalSeparator(), sg.Column(daily_times_column)],
        [sg.Button("Close", key="close")]
    ]

    window = sg.Window("Your Times", layout, element_justification="c")
